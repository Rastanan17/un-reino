...
const NPC_FRAME_ANCHO =
    NPC_SPRITE_ANCHO / NPC_COLUMNAS;

const NPC_FRAME_ALTO =
    NPC_SPRITE_ALTO / NPC_FILAS;
...
function seleccionarDialogoNPC(datos){
    ...
    agregarSeccion(
        datos.dialogos_guardia,
        4
    );

    agregarSeccion(
        datos.consejos_del_centinela,
        3
    );
    ...
    agregarSeccion(
        datos.desafios_reales,
        2
    );

    agregarSeccion(
        datos.desafios_combate,
        1
    );
    ...
    if(perfilGuardado){
        ...
        try{
            ...
            // Si no es JSON, puede ser
            // simplemente el nombre del perfil
    ...
    if(perfilActivo){
        ...
        const nivel =
            Number(perfilActivo.nivel) || 1;

        const dialogosNivel =
            datos.dialogos_por_nivel?.[nivel];
