...
function ejecutarTransicion(origen, destino, accionDestino) {
    ...
    // BUSCAR VIDEO
    // ===================================
    const videoSrc = obtenerVideoTransicion(origen, destino);
    // ===================================
    // SI NO EXISTE VIDEO
    // ===================================
    if (!videoSrc) {
        console.warn(`No existe video de transición: ${origen}-${destino}`);
    ...
    // CREAR PANTALLA
    // ===================================
    let pantalla = document.getElementById("transitionScreen");
    if (!pantalla) {
        pantalla = document.createElement("div");
        pantalla.id = "transitionScreen";
        pantalla.innerHTML = `
            <video id="transitionVideo" playsinline preload="auto"></video>
        `;
    ...
    if (!video) {
        console.error("No se pudo crear el video de transición.");
        transicionActiva = false;
        if (typeof accionDestino === "function") {
            accionDestino();
        }
    ...
    // ===================================
    const manejarError = () => {
        console.warn(`No se pudo reproducir el video: ${videoSrc}`);
        finalizar();
    };
    ...
    video.play().catch(error => {
        console.warn("El navegador bloqueó el video:", error);
        finalizar();
    });
}
// =======================================
...
function irA(origen, destino, accionDestino) {
    ejecutarTransicion(origen, destino, accionDestino);
}